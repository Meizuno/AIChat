...
const parsed = computed(() => {
  try { return JSON.parse(props.code.trim()) as NoteDetailPayload } catch { return null }
})
</script>
...
<template>
  ...
  >
    ...
    <div
      v-if="parsed.note.content"
      class="prose prose-sm dark:prose-invert max-w-none *:first:mt-0 *:last:mb-0"
    >
      <MDC
        :value="parsed.note.content"
        :cache-key="`note-detail-${parsed.note.id}`"
      />
    </div>
    <p
      v-else
